...
export async function runProtocolSuite(ctx: SuiteContext): Promise<TestResult[]> {
  ...
  if (ctx.observed.exit !== null) {
    ...
    results.push(
      fail(
        ...
        'transport',
        'spawn',
        `server exited during testing (code ${exit.code}, signal ${exit.signal ?? 'none'})`,
  ...
  try {
    ...
    results.push(
      fromError('protocol initialize', 'protocol', TestLevel.Protocol, error, 'protocol', {
        transport,
  ...
  results.push(
    pass('protocol initialize', 'protocol', TestLevel.Protocol, {
  ...
  if (session.negotiated) {
    results.push(
      warn(
        ...
        'protocol',
        TestLevel.Protocol,
        `server negotiated ${session.protocolVersion}; client claimed ${session.claimedVersion ?? 'unknown'}`,
        { protocol: session.protocolVersion, transport, durationMs: 0 },
      ),
    );
  } else {
    results.push(
      pass('protocol version-negotiation', 'protocol', TestLevel.Protocol, {
        protocol: session.protocolVersion,
        transport,
        durationMs: 0,
      }),
  ...
  if (!isProtocolVersion(session.protocolVersion) || eraOfVersion(session.protocolVersion) === null) {
    results.push(
      fail(
        ...
        'protocol',
        'version',
        `server returned unknown protocol version "${session.protocolVersion}"`,
        { protocol: session.protocolVersion, transport, durationMs: 0 },
      ),
    );
  } else {
  ...
  if (session.serverInfo.name !== undefined) {
    results.push(
      pass('protocol server-info', 'protocol', TestLevel.Protocol, {
        protocol: session.protocolVersion,
        transport,
        durationMs: 0,
  ...
  const paramsFor = (version: string): Record<string, unknown> => {
  ...
  try {
    ...
    results.push(
      warn(
        ...
        'protocol',
        TestLevel.Protocol,
        'server accepted tools/list before receiving the initialized notification',
        { transport, durationMs: ctx.now() - preStarted, evidence: list },
      ),
    );
  } catch (error) {
    if (error instanceof JsonRpcRemoteError) {
      results.push(
        pass('protocol pre-initialized-traffic', 'protocol', TestLevel.Protocol, {
          transport,
  ...
  try {
    const duplicate = await ctx.adapter.request(
      'initialize',
      paramsFor(session.claimedVersion ?? session.protocolVersion),
      requestTimeout,
    );
    results.push(
  ...
  try {
    ...
    results.push(
      warn(
        ...
        'protocol',
        TestLevel.Protocol,
        `server did not reject an unknown protocol version (${UNKNOWN_VERSION_PROBE})`,
        { transport, durationMs: ctx.now() - bogusStarted, evidence: bogus },
      ),
    );
  } catch (error) {
    if (error instanceof JsonRpcRemoteError) {
      results.push(
        pass('protocol unsupported-version', 'protocol', TestLevel.Protocol, {
          transport,
  ...
  if (ctx.observed.garbageLines.length > 0) {
    results.push(
      fail(
        ...
        'transport',
        'framing',
        `output on stdout is not valid JSON-RPC (${ctx.observed.garbageLines.length} malformed line(s))`,
        { transport, durationMs: 0, evidence: ctx.observed.garbageLines.slice(0, 5) },
      ),
    );
  } else {
  ...
  if (ctx.observed.oversize.length > 0) {
    results.push(
      warn(
        ...
        'protocol',
        TestLevel.Protocol,
        `${ctx.observed.oversize.length} line(s) exceeded the configured line size limit`,
        { transport, durationMs: 0, evidence: ctx.observed.oversize },
      ),
    );
  }
