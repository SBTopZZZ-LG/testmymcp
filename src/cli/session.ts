...
function printSessionRow(session: StoredSession): void {
...
async function createAction(
  ...
  try {
    ...
    if (transportArg === 'stdio') {
      ...
      const auth =
        commandOptions.token !== undefined
          ? { mode: 'bearer' as const, token: commandOptions.token }
          : { mode: 'none' as const };
      target = {
        transport: 'http',
        url: targetArg,
    ...
    try {
      ...
      console.error(
        `testmymcp: could not establish ${id}: ${error instanceof Error ? error.message : String(error)}`,
      );
      process.exitCode = 2;
      return;
    }
...
async function listAction(): Promise<void> {
  try {
    ...
    for (const session of sessions) printSessionRow(session);
  } catch (error) {
    console.error(`testmymcp: ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 2;
  }
...
async function showAction(idOrName: string): Promise<void> {
  try {
    const store = new SessionStore();
...
async function disposeAction(idOrName: string): Promise<void> {
  try {
    const store = new SessionStore();
