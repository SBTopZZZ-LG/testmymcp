 ...
 */
export async function runModernProtocolSuite(ctx) {
    const results = [];
    const transport = ctx.transport;
    if (ctx.observed.exit !== null) {
        results.push(fromError('protocol process-alive', 'protocol', TestLevel.Protocol, new Error(`server exited during testing`), 'transport', {
            transport,
            durationMs: 0,
            evidence: {
    ...
    try {
        ...
        results.push(fromError('protocol discover', 'protocol', TestLevel.Protocol, error, 'protocol', {
            transport,
            durationMs: ctx.now() - discoverStarted,
        }));
        results.push(warn('protocol remaining-skipped', 'protocol', TestLevel.Protocol, 'skipped because server/discover failed', {
            transport,
            durationMs: 0,
        }));
        return results;
    ...
    results.push(pass('protocol discover', 'protocol', TestLevel.Protocol, {
    ...
    if (!isProtocolVersion(session.protocolVersion) ||
        ...
        results.push(warn('protocol version-known', 'protocol', TestLevel.Protocol, `server advertised unknown protocol version "${session.protocolVersion}"`, {
            protocol: session.protocolVersion,
            transport,
            durationMs: 0,
        }));
    ...
    if (ctx.observed.garbageLines.length > 0) {
        results.push(fromError('protocol stdout-framing', 'protocol', TestLevel.Protocol, new Error(`output on stdout is not valid JSON-RPC`), 'transport', {
            transport,
            durationMs: 0,
...
//# sourceMappingURL=protocol-modern.js.map