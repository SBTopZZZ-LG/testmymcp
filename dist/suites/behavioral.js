...
function pickRoundTripTool(tools) {
    if (hasTool(tools, 'big_echo'))
        return { name: 'big_echo', argKey: 'data', extra: {} };
    if (hasTool(tools, 'delete_file'))
        return { name: 'delete_file', argKey: 'path', extra: {} };
    return undefined;
}
async function callSum(ctx, a, b, timeoutMs) {
    const result = (await ctx.adapter.request('tools/call', { name: 'sum', arguments: { a, b } }, timeoutMs));
    const text = resultText(result);
    const parsed = Number.parseInt(text, 10);
    return Number.isNaN(parsed) ? NaN : parsed;
}
export async function runBehavioralSuite(ctx) {
    const results = [];
    ...
    if (!hasTool(tools, 'sum')) {
        results.push(skip('behavioral concurrency', 'behavioral', TestLevel.Behavioral, 'no sum tool discovered', {
            transport,
            durationMs: 0,
        }));
    }
        ...
        try {
            ...
            if (mismatches.length === 0) {
                results.push(pass('behavioral concurrency parallel', 'behavioral', TestLevel.Behavioral, {
                    transport,
                    durationMs,
                ...
                results.push(fail('behavioral concurrency parallel', 'behavioral', TestLevel.Behavioral, 'application', 'response-mismatch', `${mismatches.length}/${N} parallel calls returned a result that did not match its request`, { transport, durationMs, evidence: { mismatches: mismatches.slice(0, 5) } }));
            }
        }
        catch (error) {
    ...
    results.push(ctx.adapter.mux.pendingCount === 0
    ...
    if (rt === undefined) {
        results.push(skip('behavioral payload round-trip', 'behavioral', TestLevel.Behavioral, 'no echo/delete_file tool discovered', { transport, durationMs: 0 }));
    }
    else {
        const cases = [
        ...
        for (const c of cases) {
            const started = ctx.now();
            try {
                const result = (await ctx.adapter.request('tools/call', { name: rt.name, arguments: { ...rt.extra, [rt.argKey]: c.value } }, reqTimeout));
                const text = resultText(result);
                const durationMs = ctx.now() - started;
                if (text.includes(c.value)) {
                    results.push(pass(c.id, 'behavioral', TestLevel.Behavioral, {
                        transport,
                        durationMs,
                    ...
                    results.push(fail(c.id, 'behavioral', TestLevel.Behavioral, 'application', 'payload-mismatch', 'server did not echo the exact payload back', {
                        transport,
                        durationMs,
...
function fromBehavioralError(id, error, ctx, started) {
    const durationMs = ctx.now() - started;
